...
impl FromStr for Pool {
    ...
    fn from_str(input: &str) -> Result<Pool, Self::Err> {
        match input {
            "PublicPool" => Ok(Pool::PublicPool),
            "Braiins" => Ok(Pool::Braiins),
            _ => Err(()),
        }
    }
}
impl Pool {
    fn addr(&self) -> &str {
        match self {
            Pool::PublicPool => "public-pool.io:21496",
            Pool::Braiins => "stratum.braiins.com:3333",
        }
    }
    fn username(&self) -> &str {
        match self {
            Pool::PublicPool => "1HLQGxzAQWnLore3fWHc2W8UP1CgMv1GKQ.miner1",
            Pool::Braiins => "slush.miner1",
        }
    }
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let pool = Select::new(
        ...
        Pool::iter().map(|p| p.to_string()).collect(),
    )
    .prompt()?;
    ...
    let stream =
        TcpStream::connect((*pool.addr()).to_socket_addrs().unwrap().next().unwrap()).await?;
            ...
            match c.poll_message().await {
                Ok(msg) => match msg {
                    ...
                    Some(Message::Connected) => {
                        c.send_authorize(
                            String::<64>::from_str(pool.username()).unwrap(),
                            String::<64>::from_str(pool.password()).unwrap(),
                        )
                        .await
                        .unwrap();
                    }
